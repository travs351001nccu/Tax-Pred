...
**檔案名稱**：`nowcast_H12_h{h}_YYYYMMDD.xlsx`
...
![時間序列模型預測](outputs/h_1/figures/oos_timeseries_h1_20251103.png)

**這張圖在看什麼？** - 黑色實線是實際的 YoY 成長率 -
彩色線條是各模型的預測值 - 可以目視比較哪個模型最貼近實際值

![Lasso 家族預測](outputs/h_1/figures/oos_lasso_h1_20251103.png)

**這張圖在看什麼？** - 比較 Lasso、Ridge、Elastic Net
等收縮方法的預測表現 - 通常 Adaptive 方法（紫色、橘色）會有較好的表現

![機器學習模型預測](outputs/h_1/figures/oos_ml_h1_20251103.png)
...
![模型表現排名](outputs/h_1/figures/model_ranking_h1_20251103.png)
...
![誤差分布](outputs/h_1/figures/error_distribution_h1_20251103.png)
...
![未來預測 YoY](outputs/h_1/figures/forecast_paths_yoy_h1_20251103.png)

**這張圖在看什麼？** - 深藍線是所有模型預測的平均值 - 深藍區域是 25-75
百分位數（一半模型落在此區間） - 淺藍區域是最小到最大值的範圍 -
可以看出未來 12 個月的 YoY 趨勢與不確定性

![實際稅收預測](outputs/h_1/figures/forecast_actual_h1_20251103.png)
...
![重要變數排名](outputs/h_1/figures/importance_ranking_h1_20251103.png)

**這張圖在看什麼？** - 顯示各模型認為最重要的前 5 個變數 -
連線代表同一變數在不同模型的排名 -
如果某變數出現在多個模型，表示它確實重要

![重要性熱力圖](outputs/h_1/figures/importance_heatmap_h1_20251103.png)
...
pred <- as.data.table(readxl::read_excel(
  "outputs/h_1/excel_files/forecast_outputs_h1_20251103.xlsx",
...
```         
Error: 找不到 outputs/h_1/results，請先跑對應的 0_setup.R
```
...
# 查看模型排名
library(readxl)
result <- read_excel("outputs/h_1/excel_files/forecast_outputs_h1_20251103.xlsx", 
                     sheet="model_metrics_rmse_mae_mad")
print(result)

# 查看圖表
# 直接開啟 outputs/h_1/figures/ 資料夾
```
...
# 檢視結果
list.files("outputs/h_1/excel_files/")
list.files("outputs/h_1/figures/")
```
...
│   ├── nowcast_H12_h{h}_YYYYMMDD.xlsx         # 未來預測
